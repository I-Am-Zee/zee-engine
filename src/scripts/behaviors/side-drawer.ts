...
export const sideDrawer = () => ({
  ...
  async submit() {
    ...
    if (missingSelection) {
      this.validationError = `Please select ${missingSelection.variant.name} for "${missingSelection.item.title}"`;
      return;
    }
        ...
        [item.variant_1, item.variant_2, item.variant_3].forEach(v => {
          if (v?.name && this.selections[item.id][v.name]) {
            customFields.push({
              ...
              type: 'dropdown',
              options: v.values.split(',').map((vs: any) => vs.trim()).join('|')
            });
          }
        });
