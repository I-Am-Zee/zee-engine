...
export const sideDrawer = () => ({
  ...
  init() {
    ...
    document.addEventListener("engine:item-added", (e: any) => this.openUpsell(e.detail));
    document.addEventListener("engine:quick-shop", (e: any) => this.openQuickShop(e.detail));
    document.addEventListener("engine:master-set", (e: any) => this.openMasterSet(e.detail));
  },
  ...
  openUpsell(item: any) {
    ...
    this.mode = "upsell";
    this.mainItem = item;
    this.products = (window as any).ENGINE_UPSELL_CONTEXT || [];
    ...
    try {
      ...
      for (const item of payload) {
        ...
        if (this.mode === "quick-shop" || this.mode === "master-set") {
          document.dispatchEvent(new CustomEvent("engine:show-toast", { detail: item }));
        }
      }
